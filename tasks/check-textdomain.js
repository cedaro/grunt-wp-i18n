...
grunt.registerMultiTask('checktextdomain', 'Checks gettext function calls for missing or incorrect text domain.', function() {
	...
	if( options.textdomain === false ){
		grunt.fail.warn("Text domain not provided.");
	}
	
	//Cast textdomain as an array to support multiple text domains
	options.textdomain = ( options.textdomain instanceof Array ) ? options.textdomain : [options.textdomain];
	
	//correctDomain can only be used if one domain is specified:
	options.correctDomain = options.correctDomain && ( options.textdomain.length === 1 ); 
		...
		for( var i=0; i<tokens.length; i++ ){
			...
			//Check for T_CONSTANT_ENCAPSED_STRING - and that we are in the text-domain argument
				...
				if( gettext_func.argument > 0 ){
					...
					//Correct content
					if( options.correctDomain && gettext_func.domain !== options.textdomain[0] ){
						content = "'"+options.textdomain[0]+"'";
					}
				}
			...
			//Check for variable - and that we are in the text-domain argument
				...
				if( gettext_func.argument > 0 ){
					...
					//Corect content
					if( options.reportVariableDomain && options.correctDomain ){
						content = "'"+options.textdomain[0]+"'";
					}
				}
			...
			//If in gettext function and found closing parenthesis,
				...
				if ( gettext_func.line && 0 === parens_balance ) {
					...
					if( ( options.reportVariableDomain && gettext_func.domain === -1 ) ){
						error_type = 'variable-domain'; 
							
					}else if( options.reportMissing && !gettext_func.domain ){ 
						error_type = 'missing-domain';
						
					}else if( gettext_func.domain && gettext_func.domain !== -1 && options.textdomain.indexOf( gettext_func.domain ) === -1 ) {
	...
	if ( typeof options.createReportFile === 'string' ){
        grunt.file.write( options.createReportFile, JSON.stringify( all_errors ) );
    } else if ( options.createReportFile ){
